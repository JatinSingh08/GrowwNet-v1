...
const UserPosts = async ({user}) => {
  const { links: { photos: photosApi } } = user;
        ...
        userPosts?.map(post => {
          return (
            <div key={post.id} className={styles.post__image_container}>
              <Image src={post.urls.thumb} alt={post?.description || post?.alt_description} width={200} height={200} className={styles.post__image} />
            </div>
          )
        })
      }
